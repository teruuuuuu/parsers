...
export class JBoolParser implements JParser<JBool> {
	...
	parse(input: string): ParseResult<JBool> {
		...
		if (result instanceof ParseSuccess) {
			if(result.value == "true") {
				return new ParseSuccess(new JBool(true), result.next)
			} else {
				return new ParseSuccess(new JBool(false), result.next)
			}
