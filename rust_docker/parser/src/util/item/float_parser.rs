...
fn float_test() {
    ...
    let parser = ParserGen::float();
    match parser.parse(&"abcd+1234567890.1234567efg", 4) {
    ...
    match parser.parse(&"abcd-123456789.01234567efg", 4) {
        ParseResult::Success { value, location } => {
            assert!(true);
    ...
    match parser.parse(&"abcd-0.0efg", 4) {
        ParseResult::Success { value, location } => {
            assert!(true);
            assert_eq!(0.0, value);
