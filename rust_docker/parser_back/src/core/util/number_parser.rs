...
pub fn number_parser<'a>() -> Parser<'a, i64, impl Fn(&'a [u8], usize) -> ParseResult<i64> + Clone + 'a> {
...
fn number_test() {
    ...
    match parser.parse("abcd+12345678901234567efg".as_bytes(), 4) {
    ...
    match parser.parse("abcd-0efg".as_bytes(), 4) {
        ParseResult::Success { value, location } => {
            assert!(true);
            assert_eq!(0, value);
